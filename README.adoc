...
and therefore the method expects an incoming message. Then, the message [hotspot=propertyValue file=1]`payload` is extracted
...
is updated to now return a [hotspot=sendPropertyNameHeader file=0]`Message<String>` instead of just a `String`. The
[hotspot=updateSystemProperty file=0]`updateSystemProperty()` method now returns a
[hotspot=updateSystemPropertyHeader file=0]`CompletionStage wrapped around a Response`. This allows for a response object
...
response code once the variable is completed. The variable is set to complete once the outgoing message is
[hotspot=acknowledgeAction file=0]`acknowledged`.
