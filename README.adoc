...
messages are not lost, by requiring that messages that have been delivered to the target
server are acknowledged once they have been processed. Every message that gets sent out must be acknowledged. This ensures that any messages that have been delivered
to the target service but not processed (e.g. due to a system failure) can be identified and sent again.
...
implicitly by MicroProfile Reactive Messaging. Acknowledgement for the incoming messages is controlled
by the `@Acknowledgment` annotation in MicroProfile Reactive Messaging. If the `@Acknowledgement`
...
received, and the MicroProfile Reactive Messaging acknowledgement requirement is that every message _received_ is acknowledged.

Based on the method signature, almost all of the methods in this application that require acknowledging messages are assigned
the `POST_PROCESSING` strategy by default. If the
acknowledgement strategy is set to `POST_PROCESSING`, MicroProfile Reactive Messaging acknowledges the message
...
It's important that the methods use the `POST_PROCESSING` strategy because it fulfils the requirement that messages shouldn't be acknowledged
until after the message is fully processed. This is beneficial in situations where there is a need for the
reliability of messages not getting lost. Where the `POST_PROCESSING` acknowledgement strategy can't be used, the `MANUAL` strategy can
be used with the goal of fulfilling the same requirement. In situations where message reliability is not important, and
the loss of messages is accepted, the `PRE_PROCESSING` strategy may be appropriate.

The only method in the guide that does not default to the `POST_PROCESSING` strategy is the
[hotspot=sendProperty file=0]`sendProperty` method in the `system` service. The [hotspot=sendProperty file=0]`sendProperty`
method receives property requests from the `inventory` service. For each property request, if the property being requested
is valid then it [hotspot=validReturn file=0]`returns` a property response with the value of the property. However, if the requested
property [hotspot=null file=0]`doesn't exist`, the request is ignored and no property response is
[hotspot=returnEmpty file=0]`returned`.

There is a key difference here, in the case where a property response is produced, the request is not finished processing
until the response has been sent and safely stored by the Kafka broker, and it is only then that the incoming message should be acknowledged.
...
Update the `SystemService.sendProperty` method to use the `MANUAL` acknowledgement strategy, which fits the method processing
...
annotated with the [hotspot=ackAnnotation file=0]`@Acknowledgment(Acknowledgment.Strategy.MANUAL)`
annotation. The annotation sets the method up to expect an incoming message, so the method parameter is updated to receive
and return a [hotspot=methodSignature file=0]`Message` of type `String`, instead of just a string, to fit the requirements of
acknowledgement. Then, the message [hotspot=propertyValue file=0]`payload` is extracted and checked for validity. There are two outcomes:
...
[hotspot=SPMHeader file=0]`Message<String>` instead of just a `String`. This allows the method to set a callback which
will run once the outgoing message is acknowledged. In addition to updating the [hotspot=sendPropertyName file=0]`sendPropertyName()`
method, the [hotspot=propertyNameEmitter file=0]`propertyNameEmitter` `FlowableEmitter` variable is updated to send a `Message<String>`.
...
to be returned after the outgoing message is acknowledged. The outgoing [hotspot=message file=0]`message` is created with the requested property
name as the [hotspot=payload file=0]`payload`, and an acknowledgement [hotspot=acknowledgeAction file=0]`callback` to execute
an action once the message is acknowledged. The method creates a
[hotspot=CompletableFuture file=0]`CompletableFuture` variable that returns a [hotspot=returnResult file=0]`200`
response code once the variable is completed, which happens in the [hotspot=acknowledgeAction file=0]`callback` function.
