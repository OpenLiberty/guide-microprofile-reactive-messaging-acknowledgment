...
However, if the requested property [hotspot=null file=0]`doesn't exist`, the request is ignored and no property response
is [hotspot=returnEmpty file=0]`returned`.
...
than just a `String`. Then, the message [hotspot=propertyValue file=0]`payload` is extracted and checked for validity.
...
with the requested property name as the [hotspot=payload file=0]`payload` and an acknowledgment
...
ifdef::cloud-hosted[]
If you perform the `curl` command again after a while, you notice that the CPU `systemLoad` property for the systems changed.
endif::[]
...
// Cloud hosted guide instruction
ifdef::cloud-hosted[]
Perform the `curl http://localhost:9085/inventory/systems` command again and see the `os.name` system property 
value is now included with the previous values:
endif::[]
