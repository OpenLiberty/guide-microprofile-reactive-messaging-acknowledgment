...
The application take some time to become available. After the application is up and running, 
you can access the application by making a GET request to the `/systems` endpoint of the `inventory` service.
...
When both health liveness and readiness check are up, run the following curl command to access the **inventory** microservice:
```
